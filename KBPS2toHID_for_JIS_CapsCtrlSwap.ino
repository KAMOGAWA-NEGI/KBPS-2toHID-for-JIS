/*
  ...
  特別割当:
    CapsLockキー -> 左Ctrlとして動作
    左Ctrlキー   -> CapsLockとして動作
...
bool handleCapsCtrlSwap(uint8_t ps2Code, bool make) {
  // 物理CapsLockキーを左Ctrlとして扱います。
  // CapsLockは本来Lock系キーですが、ここでは押下中だけCtrlになる通常修飾キーとして処理します。
  if (ps2Code == PS2_KEY_CAPS) {
    if (make) reportState.modifiers |= MOD_LCTRL;
    else      reportState.modifiers &= ~MOD_LCTRL;

    sendReportNow();
    return true;
  }

  // 物理左CtrlキーをCapsLockとして扱います。
  // CapsLockはトグルキーなので、Make時だけタップ送信し、Breakは消費します。
  if (ps2Code == PS2_KEY_L_CTRL) {
...
void processPs2AdvancedKey(uint16_t raw) {
  ...
  // CapsLockと左Ctrlの入れ替えを、汎用修飾キー処理より先に行います。
  if (handleCapsCtrlSwap(ps2Code, make)) {
    debugPrintKey(raw, ps2Code, HID_NONE, make, make ? "CAPS_CTRL_SWAP" : "CAPS_CTRL_SWAP_BREAK");
    return;
  }
